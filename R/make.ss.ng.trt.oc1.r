...
make.ss.ng.trt.oc1 <- function(my.df = get.ss.ng.trt.oc.df(), nlines=25, tsize=4, add.table=TRUE){
  ...
  table.plot2 <- ggplot()+
    ...
    annotate("text", label = TeX(paste0("P($\\Delta\\ \\geq$ Min TPP) > ", my.df$tau.lrv[1]*100, "% &")),
             x = 0, y = 1-3/nlines, size = tsize+1, colour = "darkgreen", hjust = 0) +
    annotate("text", label =  TeX(paste0("P($\\Delta\\,$ $\\geq$ Base TPP) > ", my.df$tau.tv[1]*100,"%")),
    ...
    annotate("text", label = TeX(paste0("P($\\Delta\\,$ $\\geq$ Min TPP) $\\leq$ ", my.df$tau.ng[1]*100, "% &")),
             x = 0, y = 1-4/nlines, size = tsize+1, colour = "red", hjust = 0) +
    annotate("text", label = TeX(paste0("P($\\Delta\\,$ $\\geq$ Base TPP) $\\leq$ ", my.df$tau.tv[1]*100,"%")),
